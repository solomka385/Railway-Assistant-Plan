...
load_dotenv(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), '.env'), encoding='utf-8')
...
def find_master_host():
    ...
    pg_db = os.getenv("PG_DB", "db1")
    pg_user = os.getenv("PG_USER", "")
    pg_password = os.getenv("PG_PASSWORD", "")
    sslmode = os.getenv("PG_SSLMODE", "verify-full")
    sslrootcert = os.getenv("PG_SSLROOTCERT", "")

    # Проверяем, что список хостов не пустой
    if not pg_hosts or not pg_hosts[0].strip():
        raise Exception("PG_HOST не задан в переменных окружения")
