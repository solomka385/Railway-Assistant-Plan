...
from scripts.config import (
    ...
    PERSIST_DIRECTORY_EMPLOYEES,
    PERSIST_DIRECTORY_SUBDIVISIONS,
...
def init_system():
    ...
    global vectorstore_employees, retriever_employees
    global vectorstore_subdivisions, retriever_subdivisions
    ...
    try:
        ...
        from scripts.database.db import init_pg_pool
        if not init_pg_pool():
        ...
        logger.info(f"[ИНИЦИАЛИЗАЦИЯ] Модель эмбеддингов: {EMBEDDING_MODEL_NAME}")
        try:
            ...
            logger.info("[ИНИЦИАЛИЗАЦИЯ] Эмбеддинг-модель загружена успешно")
        except Exception as e:
            logger.error(f"[ИНИЦИАЛИЗАЦИЯ] Ошибка загрузки эмбеддинг-модели: {e}", exc_info=True)
            raise

        logger.info("[ИНИЦИАЛИЗАЦИЯ] Начало загрузки векторных БД...")
        # Загрузка векторных БД
        reload_vectorstores()
        logger.info("[ИНИЦИАЛИЗАЦИЯ] Загрузка векторных БД завершена")

        # Загрузка примеров аварий из CSV
        from scripts.rag.ingestion import load_accidents_examples
...
def reload_vectorstores():
    ...
    logger.info("[RELOAD] Начало загрузки векторных БД...")
    
    try:
        logger.info("[RELOAD] Проверка директорий...")
        logger.info(f"[RELOAD] PERSIST_DIRECTORY_TECHNIC: {PERSIST_DIRECTORY_TECHNIC}, существует: {os.path.exists(PERSIST_DIRECTORY_TECHNIC)}")
        logger.info(f"[RELOAD] PERSIST_DIRECTORY_WORK_PLAN: {PERSIST_DIRECTORY_WORK_PLAN}, существует: {os.path.exists(PERSIST_DIRECTORY_WORK_PLAN)}")
        logger.info(f"[RELOAD] PERSIST_DIRECTORY_ALL_DOCS: {PERSIST_DIRECTORY_ALL_DOCS}, существует: {os.path.exists(PERSIST_DIRECTORY_ALL_DOCS)}")
        logger.info(f"[RELOAD] PERSIST_DIRECTORY_EMPLOYEES: {PERSIST_DIRECTORY_EMPLOYEES}, существует: {os.path.exists(PERSIST_DIRECTORY_EMPLOYEES)}")
        logger.info(f"[RELOAD] PERSIST_DIRECTORY_SUBDIVISIONS: {PERSIST_DIRECTORY_SUBDIVISIONS}, существует: {os.path.exists(PERSIST_DIRECTORY_SUBDIVISIONS)}")
        
        if os.path.exists(PERSIST_DIRECTORY_TECHNIC):
            logger.info(f"[RELOAD] Загрузка БД техники из {PERSIST_DIRECTORY_TECHNIC}...")
        ...
        if os.path.exists(PERSIST_DIRECTORY_WORK_PLAN):
            logger.info(f"[RELOAD] Загрузка БД планов работ из {PERSIST_DIRECTORY_WORK_PLAN}...")
        ...
        if os.path.exists(PERSIST_DIRECTORY_ALL_DOCS):
            logger.info(f"[RELOAD] Загрузка общей БД документов из {PERSIST_DIRECTORY_ALL_DOCS}...")
        ...
        if os.path.exists(PERSIST_DIRECTORY_EMPLOYEES):
            logger.info(f"[RELOAD] Загрузка БД сотрудников из {PERSIST_DIRECTORY_EMPLOYEES}...")
        ...
        if os.path.exists(PERSIST_DIRECTORY_EMPLOYEES_EXAMPLES):
            logger.info(f"[RELOAD] Загрузка БД примеров сотрудников из {PERSIST_DIRECTORY_EMPLOYEES_EXAMPLES}...")
            ...
            retriever_employees_examples = vectorstore_employees_examples.as_retriever(search_kwargs={"k": 5})
            logger.info("[RELOAD] Загружена БД примеров сотрудников.")
        else:
            logger.warning(f"[RELOAD] Папка {PERSIST_DIRECTORY_EMPLOYEES_EXAMPLES} не найдена.")

        if os.path.exists(PERSIST_DIRECTORY_SUBDIVISIONS_EXAMPLES):
            logger.info(f"[RELOAD] Загрузка БД примеров подразделений из {PERSIST_DIRECTORY_SUBDIVISIONS_EXAMPLES}...")
            ...
            logger.warning(f"[RELOAD] Папка {PERSIST_DIRECTORY_SUBDIVISIONS_EXAMPLES} не найдена.")

        # Загрузка БД подразделений
        if os.path.exists(PERSIST_DIRECTORY_SUBDIVISIONS):
            logger.info(f"[RELOAD] Загрузка БД подразделений из {PERSIST_DIRECTORY_SUBDIVISIONS}...")
            ...
            logger.warning(f"[RELOAD] Папка {PERSIST_DIRECTORY_SUBDIVISIONS} не найдена.")
        
        logger.info("[RELOAD] Загрузка векторных БД завершена.")
    except Exception as e:
        logger.error(f"[RELOAD] Ошибка при загрузке векторных БД: {e}", exc_info=True)
        raise
...
def batch_generate(texts):
    ...
    try:
        ...
        logger.error(f"[BATCH_GENERATE] Ошибка генерации: {e}", exc_info=True)
        return ["" for _ in texts]


def get_system_status():
    """Возвращает статус системы."""
    global model, pipe, embedding_model, vectorstore_technic, vectorstore_work_plan, vectorstore_all_docs
    global vectorstore_employees, vectorstore_subdivisions, accidents_examples
    
    return {
        "model_loaded": model is not None and pipe is not None,
        "vector_db_ready": (
            vectorstore_technic is not None or
            vectorstore_work_plan is not None or
            vectorstore_all_docs is not None or
            vectorstore_employees is not None or
            vectorstore_subdivisions is not None
        ),
        "gpu_available": torch.cuda.is_available(),
        "examples_loaded": len(accidents_examples) > 0 if accidents_examples else False
    }
