...
from scripts.config.config import (
    KEYWORDS_JSON_PATH, ACCIDENT_KEYWORDS, MAX_KEYWORDS_TO_EXTRACT
)
    ...
    try:
        if os.path.exists(KEYWORDS_JSON_PATH):
            with open(KEYWORDS_JSON_PATH, 'r', encoding='utf-8') as f:
                ...
                logger.info(f"Загружено {len(keywords)} ключевых слов из {KEYWORDS_JSON_PATH}")
                return keywords
...
def extract_keywords(description: str) -> List[str]:
    ...
    max_keywords = MAX_KEYWORDS_TO_EXTRACT
