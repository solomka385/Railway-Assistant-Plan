...
load_dotenv(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), '.env'), encoding='utf-8')

# Корневая директория проекта (rag_system)
SCRIPT_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# Путь к модели (можно переопределить через переменную окружения MODEL_PATH)
MODEL_PATH = os.getenv("MODEL_PATH", "E:/диплом/T-lite-it-2.1")
...
ALLOWED_SUBDIVISIONS = {
    ...
    "ДИЦУСИ": "Центр управления содержанием инфраструктуры",  # Альтернативное написание
    "ДИП": "дирекция инфраструктуры по пути",  # Добавлено
