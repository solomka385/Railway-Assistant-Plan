...
from scripts.rag.ingestion import reindex_documents
from scripts.rag.app_state import get_system_status
from scripts.utils.task_queue import get_task_queue, init_task_queue, shutdown_task_queue
from scripts.database import marts_db
from scripts.database.db import init_pg_pool
from scripts.rag.async_generator import get_llm_generator
...
async def ask_question(request: QueryRequest):
    ...
    try:
        ...
        if request.mode == "chat":
            ...
            # Собираем контексты для оценки RAGAS
            retrieved_contexts = []
            if subdivisions.get("sources"):
                retrieved_contexts.extend([f"Подразделение: {s}" for s in subdivisions["sources"]])
            if technics.get("sources"):
                retrieved_contexts.extend([f"Техника: {t}" for t in technics["sources"]])
            if employees.get("sources"):
                retrieved_contexts.extend([f"Сотрудник: {e}" for e in employees["sources"]])
            if work_plan.get("sources"):
                retrieved_contexts.extend([f"План работ: {w}" for w in work_plan["sources"]])
